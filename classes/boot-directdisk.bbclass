...
BOOTDD_KERNEL_APPEND ?= "root=/dev/sda2 ro"
BOOTDD_TIMEOUT ?= "5"
...
AUTO_SYSLINUXCFG = "1"
LABELS = "boot"
APPEND = "${BOOTDD_KERNEL_APPEND}"
TIMEOUT = "${BOOTDD_TIMEOUT}"
SYSLINUXCFG_IMAGE  = "${HDDDIR}/${SYSLINUXCFG}"
SYSLINUXMENU_IMAGE = "${HDDDIR}/menu"
...
build_boot_dd() {
    ...
    install -m 0644 ${SYSLINUXCFG} ${SYSLINUXCFG_IMAGE}
    if [ -f ${SYSLINUXMENU} ]; then
        install -m 0644 ${SYSLINUXMENU} ${SYSLINUXMENU_IMAGE}
    fi
